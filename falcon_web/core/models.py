...
class OfferwallProvider(models.Model):
...
class OfferwallTransaction(models.Model):
    provider = models.ForeignKey(OfferwallProvider, on_delete=models.PROTECT, related_name='transactions')
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='offerwall_transactions')
    transaction_id = models.CharField(max_length=255)
    reward = models.DecimalField(max_digits=14, decimal_places=6, validators=[MinValueValidator(Decimal('0.000001'))])
    created_at = models.DateTimeField(auto_now_add=True)
    class Meta:
        constraints = [models.UniqueConstraint(fields=['provider', 'transaction_id'], name='unique_offerwall_transaction')]
