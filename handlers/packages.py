...
BINANCE_WALLET = "TGPvUQQbd1vTgHu5mFPzVX5kah1ugX9tGN"
...
async def bronze_package(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        package_name="🥉 الباقة البرونزية",
        package_price=5
    )

    await state.set_state(
        PackageOrderState.waiting_for_post_link
    )
...
async def receive_post_link(message: Message, state: FSMContext):
    ...
    await state.set_state(
        PackageOrderState.waiting_for_payment_image
    )
    ...
    await message.answer(
        ...
        f"💰 العملة: USDT\n"
        f"🌐 الشبكة: TRC20\n\n"
        f"💰 المبلغ المطلوب: {data['package_price']} USDT\n\n"
        f"📥 عنوان المحفظة:\n"
        f"{BINANCE_WALLET}\n\n"
        f"📸 بعد التحويل أرسل صورة إثبات الدفع أو TxID."
    )
@router.message(
    PackageOrderState.waiting_for_payment_image,
    F.photo
)
async def receive_payment_image(
    message: Message,
    state: FSMContext
):
    ...
    try:
        ...
        await message.bot.send_photo(
            ...
            caption=(
                ...
                f"💰 السعر: {order.package_price} USDT\n\n"
                f"🔗 الرابط:\n{order.post_link}"
            ),
...
async def accept_package(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        if "الفضية" in order.package_name:
            target_count = 1000

        elif "الذهبية" in order.package_name:
            target_count = 2000

        elif "الماسية" in order.package_name:
            target_count = 4000
...
async def reject_package(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        await callback.message.edit_caption(
            caption=f"❌ تم رفض الطلب #{order.id}"
        )
...
async def delete_package(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        await callback.message.edit_caption(
            caption=f"🗑 تم حذف الطلب #{order_id}"
        )
