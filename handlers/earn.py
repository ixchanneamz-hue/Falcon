    ...
    try:
        ...
        for campaign in campaigns:
            ...
            await state.set_state(
                UserTaskState.waiting_for_comment
            )
            ...
            await message.answer(
                f"🎥 مهمة جديدة\n"
                f"💰 المكافأة: {campaign.reward} درهم\n"
                f"📋 التعليمات:\n"
...
async def save_comment(message: Message, state: FSMContext):
    ...
    if time.time() - start_time < 30:
        remaining = max(
            0,
            int(30 - (time.time() - start_time))
        )

        await message.answer(
            f"⏳ يرجى الانتظار {remaining} ثانية قبل إرسال التعليق."
        )
        return

    if len(comment.split()) < 3:
        await message.answer(
            "❌ يجب أن يحتوي التعليق على 3 كلمات على الأقل."
        )
        return
    ...
    await message.answer(
        "⭐ قم بتقييم الفيديو:",
        reply_markup=rating_keyboard(campaign_id)
    )
...
async def save_rating(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not campaign:
            await callback.message.answer("❌ الحملة غير موجودة.")
            await state.clear()
            return
        ...
        if user.referrer_id:
            ...
            if completed_tasks >= 5:
                ...
                if referrer:
                    ...
                    commission = campaign.reward * 0.10
        ...
        await callback.message.edit_text(
            ...
            f"⭐ التقييم: {rating}/5\n"
            f"💰 تمت إضافة {campaign.reward} درهم إلى رصيدك."
        )
    finally:
        db.close()
    await state.clear()
